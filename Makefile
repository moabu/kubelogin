...
clean: ## Remove previous build
	rm -f $(DARWIN_ZIP) $(DARWIN) $(LINUX_ZIP) $(LINUX) $(WINDOWS_ZIP) $(WINDOWS) *.sha256.sum

help: ## Display available commands
	@grep -E '^[a-zA-Z_-]+:.*?## .*$$' $(MAKEFILE_LIST) | awk 'BEGIN {FS = ":.*?## "}; {printf "\033[36m%-30s\033[0m %s\n", $$1, $$2}'

build-local:
		go build -v -o /usr/local/bin/kubelogin -ldflags="-s -w -X main.version=$(VERSION)"  ./*.go